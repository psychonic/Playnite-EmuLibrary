...
namespace EmuLibrary.RomTypes.Yuzu
{
    class Yuzu
    {
        ...
        public string UserPath
        {
            get
            {
                var portableUserDir = Path.Combine(BasePath, "user");
                if (Directory.Exists(portableUserDir))
                    return portableUserDir;
        ...
        private void TryLoadKeysFromEmulatorOrHactool(Keyset keySet, KeyFileType fileType, string fileName, KeySource source)
        {
            string keyPath = null;

            if (source.HasFlag(KeySource.Emulator))
            {
        ...
        // Builds a fresh Keyset from the emulator/hactool key files. Scan workers each get their own via
        // ThreadLocal because ImportTickets mutates Keyset.ExternalKeySet (a non-thread-safe dictionary read
        // during NCA decryption), so the shared instance KeySet cannot be used across parallel Map workers.
        private Keyset BuildKeyset()
        {
            var keySet = new Keyset();
            TryLoadKeysFromEmulatorOrHactool(keySet, KeyFileType.Standard, "prod.keys", KeySource.Emulator | KeySource.Hactool);
            TryLoadKeysFromEmulatorOrHactool(keySet, KeyFileType.Standard, "prod.keys_autogenerated", KeySource.Emulator);
            TryLoadKeysFromEmulatorOrHactool(keySet, KeyFileType.Title, "title.keys", KeySource.Emulator | KeySource.Hactool);
            TryLoadKeysFromEmulatorOrHactool(keySet, KeyFileType.Title, "title.keys_autogenerated", KeySource.Emulator);
            TryLoadKeysFromEmulatorOrHactool(keySet, KeyFileType.Console, "console.keys", KeySource.Emulator | KeySource.Hactool);
            TryLoadKeysFromEmulatorOrHactool(keySet, KeyFileType.Console, "console.keys_autogenerated", KeySource.Emulator);
            return keySet;
        }
        ...
        private void SaveKeys()
        {
            ...
            sb.AppendLine("# This file is autogenerated by EmuLibrary");
        ...
        private void ImportTickets(PartitionFileSystem fs, Keyset keySet)
        {
        ...
        private readonly IScanConcurrency _scanConcurrency;

        public Yuzu(string basePath, ILogger logger, IScanCache scanCache = null, IScanConcurrency scanConcurrency = null)
        {
            BasePath = basePath;
            (_processName, _appDataFolder) = GetEmulatorConfig(basePath);
            _logger = logger;
            _scanCache = scanCache;
            _scanConcurrency = scanConcurrency;
        ...
        public void InstallFileToNand(string fileToInstall)
        {
            if (IsEmulatorRunning())
                throw new Exception($"Cannot install game while {_processName} is running");
                    ...
                    using (var outStream = File.Open(outPath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None))
                    {
                        ...
                        else
                        {
                            ...
                            using (var br = new BinaryReader(inStream))
                            {
                                ...
                                var ncaHeader = br.ReadBytes(0x4000);
        ...
        {
            if (IsEmulatorRunning())
                throw new Exception($"Cannot uninstall game while {_processName} is running");
        ...
        // Parses one cnmt NCA in a partition into its (self-contained) Cnmt and computes its install
        // footprint, without the full Control/Program/NACP extraction. Used to size every content unit in a
        // multi-cnmt file and to hand the primary unit's already-parsed Cnmt back to the caller, so it isn't
        // opened and parsed a second time. The returned Cnmt stays valid after the NCA streams are disposed.
        private bool TryReadCnmt(PartitionFileSystem pfs, DirectoryEntryEx fileEntry, Keyset keySet, out Cnmt cnmt, out ulong installSize)
        {
            cnmt = null;
            installSize = 0;
            ...
            using (ncaFile)
            {
                ...
                using (var fs = cnmtNca.OpenFileSystem(NcaSectionType.Data, IntegrityCheckLevel.IgnoreOnInvalid))
                {
                    ...
                    if (fs.OpenFile(out var metaFile, cnmtPath, OpenMode.Read).IsSuccess())
                    {
                        using (metaFile)
                        {
                            cnmt = new Cnmt(metaFile.AsStream());
        ...
        private ExternalGameFileInfo ProcessInstalledGameFromCmntNca(Nca cnmtNca, Keyset keySet)
        {
            ...
            using (var fs = cnmtNca.OpenFileSystem(NcaSectionType.Data, IntegrityCheckLevel.IgnoreOnInvalid))
            {
                ...
                if (fs.OpenFile(out var metaFile, cnmtPath, OpenMode.Read).IsSuccess())
                {
                    using (metaFile)
                    {
                        ...
                        {
                            // entry.Type already classifies each content unit, so the Program NCA's path comes
                            // from its NcaId without opening it, and Data/logo/legal NCAs are skipped entirely.
                            // Only the Control NCA is opened (for the NACP).
                            foreach (var entry in cnmt.ContentEntries)
                            {
                                if (entry.Type == LibHac.Ncm.ContentType.Program)
                                {
                                ...
                                if (entry.Type != LibHac.Ncm.ContentType.Control)
                                    continue;
        ...
        // Runs the per-file inspection over `items` through the endpoint-aware scan concurrency governor when
        // one is available (parallel, bounded per-host + global), falling back to a sequential pass otherwise
        // (install/uninstall controllers and the benchmark construct Yuzu without a governor). `worker` is
        // self-contained (own try/catch); null results are kept and filtered by the caller.
        private IReadOnlyList<TResult> RunInspections<TItem, TResult>(
            IReadOnlyList<TItem> items, string endpointPath, Func<TItem, TResult> worker, CancellationToken tk)
        {
            if (_scanConcurrency != null)
                return _scanConcurrency.Map(items, endpointPath, worker, tk);
        ...
        public IEnumerable<YuzuInstalledTitle> GetInstalledGames(CancellationToken tk)
        {
            ...
            var files = new SafeFileEnumerator(installedNcaDir, "*.nca", SearchOption.AllDirectories).ToList();

            // Inspect every NAND NCA in parallel through the governor (keyed by the registered dir, usually
            // local), each worker using its own keyset. Group-by/aggregation runs single-threaded afterward.
            IReadOnlyList<ExternalGameFileInfo> infos;
            using (var threadKeyset = new ThreadLocal<Keyset>(BuildKeyset))
            {
                infos = RunInspections(files, installedNcaDir, file =>
                {
                    using (var ncaFile = File.Open(file.FullName, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        var cnmtNca = new Nca(threadKeyset.Value, ncaFile.AsStorage());
                        ...
                        {
                            try { info = ProcessInstalledGameFromCmntNca(cnmtNca, threadKeyset.Value); }
            ...
            if (!tk.IsCancellationRequested)
                _scanCache?.RemoveKeysUnder(installedNcaDir, new HashSet<string>(files.Select(f => f.FullName), StringComparer.OrdinalIgnoreCase));
        ...
        // Single-threaded entry point: uses the instance KeySet. The parallel scan path calls the Keyset
        // overload with a per-thread keyset instead (see GetUninstalledGamesFromDir).
        public ExternalGameFileInfo GetExternalGameFileInfo(string filePath)
        {
            return GetExternalGameFileInfo(filePath, KeySet);
        }

        private ExternalGameFileInfo GetExternalGameFileInfo(string filePath, Keyset keySet)
        {
            var info = new ExternalGameFileInfo();
            bool haveCnmt = false;
            bool haveNacp = false;
            ...
            using (var fileStream = fileInfo.OpenRead())
            {
                ...
                using (pfs)
                {
                    ...
                    // cnmt, so pick the Application one (falling back to the first) as the primary, keeping its
                    // already-parsed Cnmt so it isn't re-opened and re-parsed below.
                    Cnmt cnmt = null;
                    ...
                    foreach (var ce in fileEntries)
                    {
                        try
                        {
                            if (!TryReadCnmt(pfs, ce, keySet, out var ceCnmt, out var ceSize))
                                continue;
                            totalInstallSize += ceSize;
                            bool isApp = ceCnmt.Type == LibHac.Ncm.ContentMetaType.Application;
                            if (cnmt == null || (isApp && !primaryIsApp))
                            {
                                cnmt = ceCnmt;
                                primaryIsApp = isApp;
                            }
                    ...
                    if (info.Type != ExternalGameFileType.DLC)
                    {
                        // The cnmt already classifies each content unit, so the Program NCA's launch path is
                        // derivable from its NcaId without opening it, and Data/logo/legal NCAs need not be
                        // opened at all. Only the Control NCA is read (for the NACP title/publisher/version).
        ...
        public IEnumerable<YuzuTitle> GetUninstalledGamesFromDir(string path, CancellationToken tk)
        {
            ...
            var files = new SafeFileEnumerator(path, "*.*", SearchOption.AllDirectories)
                .Where(f => (!f.Attributes.HasFlag(FileAttributes.Directory)) && ValidGameExtensions.Contains(f.Extension))
                .ToList();

            // Inspect every source file in parallel through the governor (keyed by the source dir), each worker
            // using its own keyset since ImportTickets mutates it. Group-by/aggregation runs single-threaded.
            IReadOnlyList<ExternalGameFileInfo> infos;
            using (var threadKeyset = new ThreadLocal<Keyset>(BuildKeyset))
            {
                infos = RunInspections(files, path, file =>
                {
                    ...
                    else
                    {
                        try
                        {
                            extGameInfo = GetExternalGameFileInfo(file.FullName, threadKeyset.Value);
                        }
                        catch (Exception ex)
                        {
            ...
            foreach (var extGameInfo in infos)
            {
                if (extGameInfo == null)
                    continue;
            ...
            if (!tk.IsCancellationRequested)
                _scanCache?.RemoveKeysUnder(path, new HashSet<string>(files.Select(f => f.FullName), StringComparer.OrdinalIgnoreCase));
