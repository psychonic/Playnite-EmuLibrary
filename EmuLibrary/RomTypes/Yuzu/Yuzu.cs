...
namespace EmuLibrary.RomTypes.Yuzu
{
    class Yuzu
    {
        ...
        private void TryLoadKeysFromEmulatorOrHactool(KeyFileType fileType, string fileName, KeySource source)
        {
            string keyPath = null;
        ...
        {
            KeySet = new Keyset();
            TryLoadKeysFromEmulatorOrHactool(KeyFileType.Standard, "prod.keys", KeySource.Emulator | KeySource.Hactool);
            TryLoadKeysFromEmulatorOrHactool(KeyFileType.Standard, "prod.keys_autogenerated", KeySource.Emulator);
            TryLoadKeysFromEmulatorOrHactool(KeyFileType.Title, "title.keys", KeySource.Emulator | KeySource.Hactool);
            TryLoadKeysFromEmulatorOrHactool(KeyFileType.Title, "title.keys_autogenerated", KeySource.Emulator);
            TryLoadKeysFromEmulatorOrHactool(KeyFileType.Console, "console.keys", KeySource.Emulator | KeySource.Hactool);
            TryLoadKeysFromEmulatorOrHactool(KeyFileType.Console, "console.keys_autogenerated", KeySource.Emulator);
        }
        ...
        private bool TryReadCnmt(PartitionFileSystem pfs, DirectoryEntryEx fileEntry, out Cnmt cnmt, out ulong installSize)
        {
            cnmt = null;
            installSize = 0;
            ...
            using (var fileStream = fileInfo.OpenRead())
            {
                ...
                using (pfs)
                {
                    ...
                    foreach (var ce in fileEntries)
                    {
                        try
                        {
                            if (!TryReadCnmt(pfs, ce, out var ceCnmt, out var ceSize))
                                continue;
